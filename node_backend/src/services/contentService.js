...
const assertModuleAccess = async ({ currentUser, instituteId, moduleItem, forWrite }) => {
  ...
  const pairKey = `${asId(moduleItem.courseId)}::${asId(moduleItem.subcourseId)}`;
  if (hasRole(currentUser, "teacher")) {
    ...
    if (!teacherScope.coursePairs.has(pairKey)) {
      throw new AppError("Teachers can only access content for assigned courses.", 403);
    }
    return;
  }
  ...
  if (!forWrite && hasRole(currentUser, "student")) {
    ...
    if (!studentScope.coursePairs.has(pairKey)) {
      throw new AppError("Students can only access content for enrolled courses.", 403);
    }
    return;
  }
...
const assertContentWriteAccess = async (currentUser, content) => {
...
const createContent = async (payload, file, tenant, user) => {
  ...
  await assertModuleAccess({ currentUser: user, instituteId, moduleItem, forWrite: true });

  const upload = file ? await uploadFile(file, instituteId, payload.module_id) : null;
  try {
...
const listModuleContents = async (moduleId, tenant, currentUser) => {
...
const updateContent = async (id, payload, file, tenant, currentUser) => {
  ...
  await assertContentWriteAccess(currentUser, content);

  const uploaded = file ? await uploadFile(file, content.instituteId.toString(), content.moduleId.toString()) : null;
