...
def _include_inactive_for_user(db: Session, current_user: User) -> bool:
    return "super_admin" in _role_names(db, current_user)
...
def _student_scope(
    ...
    assignments = enrollment_crud.list_user_courses(db, current_user.user_id, institute_id)
    course_ids = {assignment.course_id for assignment in assignments}
    course_pairs = {(assignment.course_id, assignment.subcourse_id) for assignment in assignments}
    return course_ids, course_pairs
...
def list_courses(db: Session, tenant: TenantContext, current_user: User) -> list[Course]:
    return list_courses_for_institute(db, tenant, tenant.institute_id, current_user)
...
def list_courses_for_institute(
    ...
    if "student" in role_names and "super_admin" not in role_names and "institute_admin" not in role_names:
        course_ids, _ = _student_scope(db, current_user, institute_id)
        return [course for course in courses if course.active and course.course_id in course_ids]
    return courses
...
def list_subcourses(
    ...
    return list_subcourses_for_institute(db, tenant, tenant.institute_id, current_user, course_id)
...
def list_subcourses_for_institute(
    ...
    if "student" in role_names and "super_admin" not in role_names and "institute_admin" not in role_names:
        _, course_pairs = _student_scope(db, current_user, institute_id)
        return [
            subcourse
            for subcourse in subcourses
            if subcourse.active and (subcourse.course_id, subcourse.subcourse_id) in course_pairs
...
def list_modules(
    ...
    return list_modules_for_institute(
        db, tenant, tenant.institute_id, current_user, course_id=course_id, subcourse_id=subcourse_id
    )
...
def delete_course(db: Session, course_id: str, tenant: TenantContext, current_user: User) -> None:
    course = course_crud.get_course(
        db, course_id, resolve_institute_scope(db, current_user, tenant)
    )
    if course is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found.")
    try:
...
def delete_subcourse(
    db: Session, subcourse_id: str, tenant: TenantContext, current_user: User
) -> None:
    subcourse = course_crud.get_subcourse(
        db, subcourse_id, resolve_institute_scope(db, current_user, tenant)
    )
    if subcourse is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Subcourse not found.")
    try:
